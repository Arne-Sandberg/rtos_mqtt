...
{  
	...
	const TickType_t  xTimerPer = 100;	//tick = 1ms,100ms ÊØè100ms ÂñÇÁãó

	__set_PRIMASK(1);//Ω˚÷π»´æ÷÷–∂œ
	//prvSetupHardware();	
	System_Setup();
	...
	printf("\r\n\r\n");
	  ...
	  /* Ê£ÄÊü•ÊòØÂê¶‰∏∫Áã¨Á´ãÁúãÈó®ÁãóÂ§ç‰Ωç */
	if (RCC_GetFlagStatus(RCC_FLAG_IWDGRST) != RESET)
	{
	  /* Áã¨Á´ãÁúãÈó®ÁãóÂ§ç‰Ωç */
	...
	wdgTimerHandle = xTimerCreate("wdg",		   /* ÂÆöÊó∂Âô®ÂêçÂ≠ó */
							   xTimerPer,	 /* ÂÆöÊó∂Âô®Âë®Êúü,Âçï‰ΩçÊó∂ÈíüËäÇÊãç */
							   pdTRUE,			/* Âë®ÊúüÊÄß */
							   ( void * ) 0,
							   wdgTimerCallback); /* ÂÆöÊó∂Âô®ÂõûË∞ÉÂáΩÊï∞ */

	if(wdgTimerHandle == NULL)
	{
		/* Ê≤°ÊúâÂàõÂª∫ÊàêÂäüÔºåÁî®Êà∑ÂèØ‰ª•Âú®ËøôÈáåÂä†ÂÖ•ÂàõÂª∫Â§±Ë¥•ÁöÑÂ§ÑÁêÜÊú∫Âà∂ */
	   printf("watchdog timer fail!!\r\n");
	}
	else
	{
		 /* ÂêØÂä®ÂÆöÊó∂Âô®ÔºåÁ≥ªÁªüÂêØÂä®ÂêéÊâçÂºÄÂßãÂ∑•‰Ωú */
		 if(xTimerStart(wdgTimerHandle, 10) != pdPASS)//Á≠âÂæÖÂª∂Êó∂100ms
		 	{
			 /* ÂÆöÊó∂Âô®ËøòÊ≤°ÊúâËøõÂÖ•ÊøÄÊ¥ªÁä∂ÊÄÅ */
		 	}
	 }
