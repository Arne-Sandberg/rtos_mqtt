...
u8 SD_GetResponse(u8 Response)
{
	...
	while((SPI1_ReadWriteByte(0XFF)!=Response)&&Count)	 // 等待得到准确的回应
		Count--;	  	  
	if(Count==0)
...
u8 SD_WaitDataReady(void)
{
	...
	do
	{
		r1 = SPI1_ReadWriteByte(0xFF) & 0X1F;	// 读到回应
		if(retry == 0xFFFE)
			return 1; 
		retry++;
		switch (r1)
	...
	retry=0;
	while(SPI1_ReadWriteByte(0XFF)==0)//读到数据为0,则数据还未写完成
		{
		retry++;
...
u8 SD_SendCommand(u8 cmd, u32 arg, u8 crc)
{
	...
	u8 Retry=0;	         
	Set_SD_CS;
	SPI1_ReadWriteByte(0xff);//高速写命令延时
...
u8 SD_ReadMultiBlock(u32 sector, u8 *buffer, u8 count)
	{
	u8 r1;	 			 
	//SPI1_SetSpeed(SPI_SPEED_HIGH);//设置为高速模式  
...
u8 SD_Read_Bytes(unsigned long address,unsigned char *buf,unsigned int offset,unsigned int bytes)
	...
	for(i=0;i<offset;i++)SPI1_ReadWriteByte(0xff);//跳过offset位 
	for(;i<offset+bytes;i++)*buf++=SPI1_ReadWriteByte(0xff);//读取有用数据	
	for(;i<512;i++) SPI1_ReadWriteByte(0xff); 	 //读出剩余字节
	SPI1_ReadWriteByte(0xff);//发送伪CRC码
	SPI1_ReadWriteByte(0xff);  
	Set_SD_CS;//关闭SD卡
	return 0;
	}
